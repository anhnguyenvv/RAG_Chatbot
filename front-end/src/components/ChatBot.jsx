...
import robot_img from "../assets/chatbot.jpg";
...
function ChatBot(props) {
  ...
  let [sourceData, SetSourceData] = useState("qdrant");
  let [chatHistory, SetChatHistory] = useState([]);

  const commonQuestions=[
    "Sinh viên phải đăng ký tối thiểu bao nhiêu tín chỉ trong học kỳ chính?",
    "Bao nhiêu điểm thì học lực Xuất sắc?",
    "Tổng số tín chỉ bắt buộc của ngành/Chuyên ngành là bao nhiêu?",
    "Liệt kê một số môn học bắt buộc của chuyên ngành Công nghệ tri thức?",
    "Điều kiện thực hiện đề tài tốt nghiệp là gì?",
    "Sinh viên cần chuẩn trình độ ngoại ngữ là bao nhiêu để tốt nghiệp?",
    "Chương trình đào tạo bao gồm bao nhiêu tín chỉ Giáo dục đại cương bắt buộc?",
    "Sinh viên cần làm gì để thực hiện đề tài khác chuyên ngành?",
    "Môn học nào thuộc Học kỳ 1 năm 4 chuyên ngành Khoa học dữ liệu?",
    "Nội dung môn Xử lý ngôn ngữ tự nhiên là gì?",
    "Thời gian các tiết học tại cơ sở 2",
  ...
  const [dataChat, SetDataChat] = useState([
    [
      "start",
      [
        "Xin chào! Đây là FITHCMUS Chatbot, trợ lý đắc lực dành cho bạn! Bạn muốn tìm kiếm thông tin về những gì? Đừng quên chọn nguồn tham khảo phù hợp để mình có thể giúp bạn tìm kiếm thông tin chính xác nhất nha. 😄",
        null,
      ],
    ],
  ]);
  ...
  async function SendMessageChat() {
    if (promptInput !== "" && isLoading === false) {
      ...
      fetch("https://hugely-immense-bedbug.ngrok-free.app/rag/" + sourceData + "?q=" + promptInput,
      {
        method: "get",
        headers: new Headers({
                        ...
                        {dataMessages[1][1].map((source, j) => (
                          ...
                          >
                            {dataMessages[1][2] == "wiki"
                              ...
                              : source.metadata.page==undefined? "Chương trình đào tạo khóa 2023" : "Trang " +
                ...
                <div className="chat-bubble shadow-xl chat-bubble-primary bg-gradient-to-r from-purple-500 to-blue-500 text-white">
                  ...
                  <>
                    ...
                    <p className="font-light text-xs text-cyan-50">
                      Tham khảo:{" "}
                      {dataMessages[1][1] == "wiki" ? "Wikipedia" : "FITHCMUS"}
                    </p>
                  </>
                </div>
              </div>
